...
export default function DateTimePickerModal({
  ...
  // ✅ FIX 1: Use current month instead of hardcoded Dec 2025
  ...
  // ✅ FIX 2: Auto-navigate to clicked date's month
  const handleDateSelect = (date: Dayjs) => {
    ...
    setEditingDate(date);
    // 🔥 Auto-switch calendar to the selected date's month
    setSelectedMonth(date.startOf("month"));
  };

  const handleDateDoubleClick = (date: Dayjs) => {
    setEditingDate(date);
    setSelectedMonth(date.startOf("month")); // also navigate on double-click
  };
    ...
    >
      <div style={{ padding: "24px" }}>
        <div style={{ textAlign: "center", marginBottom: "24px" }}>
          ...
          {/* ✅ Dynamically show current viewed month */}
          <Text type="secondary">{selectedMonth.format("MMMM YYYY")}</Text>
        </div>
        ...
        <Row gutter={24}>
          <Col span={12}>
            <Calendar
              ...
              onSelect={handleDateSelect} // ✅ Keep this — it's correct
                  ...
                  >
                    ...
                    {currentDateSchedules.length > 0 && (
                      <Button
                        ...
                        danger
                        onClick={() => handleRemoveAllSchedulesForDate(editingDate.format("YYYY-MM-DD"))}
                      >
                        Clear All
                      </Button>
                    )}
